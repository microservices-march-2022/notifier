...
POSTGRES_USER=$(curl -X GET "http://${CONSUL_HOST}:${CONSUL_PORT}/v1/kv/notifier-db-application-user?raw=true")
...
PGPASSWORD=$(curl -X GET "http://${CONSUL_HOST}:${CONSUL_PORT}/v1/kv/notifier-db-password-never-do-this?raw=true")
...
docker run \
  ...
  # CONSUL_CLIENT_HOST=consul-client CONSUL_CLIENT_PORT=8500 CONSUL_SERVER_HOST=localhost CONSUL_SERVER_PORT=8500 ./infrastructure/notifier-deploy.sh